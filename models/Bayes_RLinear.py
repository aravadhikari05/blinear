    ...
    def __init__(self, configs):
        ...
        if self.individual:
            self.layers = nn.ModuleList([
                BayesianLinear(configs.seq_len, configs.pred_len) for _ in range(configs.channel)
            ])
        else:
            self.layer = BayesianLinear(configs.seq_len, configs.pred_len)
    ...
    def forward(self, x, y):
        ...
        if self.individual:
            ...
            for idx, layer in enumerate(self.layers):
                pred[:, :, idx] = layer(x[:, :, idx])
                kl_total += layer.kl_loss()
        else:
            # Use same weights across all channels
            x_transposed = x.transpose(1, 2)  # [B, D, L]
            out = self.layer(x_transposed)    # [B, D, P]
            pred = out.transpose(1, 2)        # [B, P, D]
            kl_total = self.layer.kl_loss()

        pred = self.rev(pred, 'denorm') if self.rev else pred
        mse = F.mse_loss(pred, y)
        return pred, mse, kl_total
